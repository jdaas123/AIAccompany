...
def add_message_to_buffer(data,buffer):
    ...
    if message_type in ["text","face"]:
        buffer["messages"].append({"type":message_type,"content":data["raw_message"]})
    else:
        buffer["messages"].append({"type":message_type,"content":message[0]["data"]["url"]})
